...
We use our prior knowledge of $k=9$ clusters to cut the trees and hopefully get a more clear signal.
...
m2 <- cutree(hcc, k = 9)
m3 <- cutree(hca, k = 9)
hcsm <- hclust(dist(raw), method = "single", members = m1)
hccm <- hclust(dist(raw), method = "complete", members = m2)
hcam <- hclust(dist(raw), method = "average", members = m3)
plot(hcsm, main = "Single-Linkage Dendogram")
plot(hccm, main = "Complete-Linkage Dendogram")
plot(hcam, main = "Average-Linkage Dendogram")
